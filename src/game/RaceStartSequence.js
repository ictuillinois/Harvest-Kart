...
// ── Pre-inject CSS at module load time (avoids layout stall during countdown) ──
if (!document.getElementById('race-start-styles')) {
  const _style = document.createElement('style');
  _style.id = 'race-start-styles';
  _style.textContent = `
    /* ── Countdown number ── */
    #countdown-display {
...
export class RaceStartSequence {
  ...
  start() {
    ...
    // ── Countdown + traffic light (single appendChild via fragment to avoid 2 reflows) ──
    ...
    frag.appendChild(this._countdownEl);
    frag.appendChild(this._trafficEl);
